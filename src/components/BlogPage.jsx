    ...
    return (
        <div class='page'>
            ...
            <div className='three-col-grid'>
                ...
                <BlogPost image={logo} isSquare={true} date="2/05/2026" title='Origins' text="How did we become a community of Tall Horses, you ask? A game of jackbox, a google form, and a non-insignificant amount of alcohol." />
            </div>
        </div>
    )
}

function BlogPost({ image, isSquare=false, date, title, text }) {
    return (
        <div>
            <Image src={image} isSquare={isSquare} isBordered={true}/>            
            <h3>{date}</h3>
            <h3>{title}</h3>
            <p>{text}</p>
        </div>
